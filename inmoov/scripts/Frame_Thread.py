...
def interpolate(dest, curr, dur):
    # find the delta(s)
    delta = float(dest - curr)
    # total time must not be less than INTERPOLATE_TIME
    dur = float(max(dur, INTERPOLATE_TIME))
    ...
    frame_time = dur / float(num_frames)
    ...
    frame_list = [[(float(i+1)/float(num_frames) * delta) + curr, frame_time] for i in range(num_frames)]
